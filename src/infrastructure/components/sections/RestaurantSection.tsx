...
import CardRestaurantPublic from '../menu/CardRestaurantPublic';

export default function RestaurantSection() {
    const [page, setPage] = useState(1)
    const [filter, setFilter] = useState<{ environment?: string, type?: string, param?: string, landing:boolean }>({landing:true});
    ...
    return (
        <section className="min-h-screen max-w-lg mx-auto mt-24">
            ...
            <div className="space-y-6">
                {entity.list.map((item) => <CardRestaurantPublic key={item.id} item={item} />)}
            </div>
        </section>
    );
}