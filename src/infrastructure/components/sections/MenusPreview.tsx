...
const MenusPreview = () => {
  ...
  const menuHooks = useMenusCrud(1, 4);
    ...
    <div>
      {
        menuHooks.loading
          ...
          : menuHooks.error
            ...
            : menuHooks.list && <section className="py-16 md:py-24" id="how-it-works">
              <div className="container mx-auto px-4 md:px-6">
                <div className="text-center mb-16">
                  <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-gray-100 mb-4">Alguno de nuestros menús</h2>
                  <p className="text-xl text-gray-600 max-w-3xl mx-auto">
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                  {menuHooks.list.map((menu, index) => (
                    <div
                      key={index}
                    ...
                    >
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">{menu.name}</h3>
                      <p className="text-gray-700 dark:text-gray-300">{menu.about.substring(0, 50)} {menu.about.length > 50 && `...`}</p>
                    </div>
                  ))}
                </div>
              </div>
