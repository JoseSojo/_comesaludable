...
import CategoryForm from '@/infrastructure/components/form/core/CategoryForm';
import CategoryFicha from '@/infrastructure/components/core/unique/CategoryUnique';
...
export default function CategoryPage() {
    ...
    const entity = useCategoryCrud(page, 10, filter);
    ...
    return (
        <>
            <Modal isOpen={modalForm} onClose={() => setModalForm(true)}>
                <CategoryForm initialData={null} end={() => setModalForm(false)} />
            </Modal>
            {
                selectId && <Modal isOpen={selectId ? true : false} onClose={() => setSelectId(null)}>
                    <CategoryFicha close={() => setSelectId(null)} id={selectId} />
                </Modal>
            }

            {
                selectIdDelete && <Modal isOpen={selectId ? true : false} onClose={() => setSelectId(null)}>
                    <DeleteAlert deleteFn={()=> {entity.deleteCategory(selectIdDelete); setSelectIdDelete(null)}} />
                </Modal>
            }
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="dark:text-gray-50 text-2xl font-bold">Categorías</h1>
                        <p className="text-gray-500 dark:text-gray-400">Gestionar todas las categorías que forman parte de come saludable.</p>
                    </div>
                </div>
                    ...
                    <table className="min-w-full bg-white dark:bg-gray-800 rounded-lg overflow-hidden">
                        <thead className="bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-300 text-sm">
                            <tr>
                                ...
                                <th className="py-3 px-4 text-left font-medium">Menus</th>
                                ...
                                : <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-gray-700 dark:text-gray-300">
                                    {entity.list.map((item) => (
                                        <tr key={item.id} className="">
                                            ...
                                            <td className="py-3 px-4 font-medium">{item._count.menus}</td>
