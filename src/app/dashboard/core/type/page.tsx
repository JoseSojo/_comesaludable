...
import TypeForm from '@/infrastructure/components/form/core/TypeFrom';
import Button from '@/infrastructure/components/ui/Button';
import TypeFicha from '@/infrastructure/components/core/unique/TypeUnique';
...
export default function TypePage() {
    ...
    const entity = useTypeCrud(page, 10, filter);
    ...
    return (
        <>
            <Modal isOpen={modalForm} onClose={() => setModalForm(true)}>
                <TypeForm initialData={null} end={() => setModalForm(false)} />
            </Modal>
            {
                selectId && <Modal isOpen={selectId ? true : false} onClose={() => setSelectId(null)}>
                    <TypeFicha close={()=>setSelectId(null)} id={selectId} />
                </Modal>
            }

            {
                selectIdDelete && <Modal isOpen={selectId ? true : false} onClose={() => setSelectId(null)}>
                    <DeleteAlert deleteFn={()=> {entity.deleteType(selectIdDelete); setSelectIdDelete(null)}} />
                </Modal>
            }
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        <h1 className="dark:text-gray-50 text-2xl font-bold">Tipos</h1>
                        <p className="text-gray-500 dark:text-gray-400">Gestionar todos los tipos.</p>
                    </div>
                </div>
